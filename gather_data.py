...
###  TODO: MOVE EVERYTHING BELOW TO A NEW ANALYSIS SCROPT

# 1. Calculate how many citations per article in covid vs noncovid vs top N noncovid
citedBy_count_cov = [len(a.citedByPMIDs) for a in query_covid.articles]
citedBy_count_noncov = [len(a.citedByPMIDs) for a in query_noncovid.articles]
citedBy_count_precov = [len(a.citedByPMIDs) for a in query_precovid.articles]


#  - get the top noncov papers and count their citations
citedBy_count_noncov_top = citedBy_count_noncov
citedBy_count_noncov_top.sort(reverse=True)
citedBy_count_noncov_top = citedBy_count_noncov_top[:len(citedBy_count_cov)]


#  - get the top precov papers and count their citations
citedBy_count_precov_top = citedBy_count_precov
citedBy_count_precov_top.sort(reverse=True)
citedBy_count_precov_top = citedBy_count_precov_top[:len(citedBy_count_cov)]

print('='*70)
print('OVERALL CITATIONS PER ARTICLE')
print('Citations per article: {:.2f} COVID vs {:.2f} Non-COVID vs {:.2f} Top {} Non-COVID'.format(
                            np.mean(citedBy_count_cov),
                            np.mean(citedBy_count_noncov),
                            np.mean(citedBy_count_noncov_top),
                            len(citedBy_count_noncov_top)))

print('Citations per article: {:.2f} Pre-COVID vs {:.2f} Top {} Pre-COVID'.format(
                            np.mean(citedBy_count_precov),
                            np.mean(citedBy_count_precov_top),
                            len(citedBy_count_precov_top)))
...
citedBy120Days_covid = limit_citedBy_time(query_covid.articles, DAYS_CITEDBY_LIMIT)
citedBy120Days_noncovid = limit_citedBy_time(query_noncovid.articles, DAYS_CITEDBY_LIMIT)
citedBy120Days_precovid = limit_citedBy_time(query_precovid.articles, DAYS_CITEDBY_LIMIT)


citedBy_count_cov = [len(citedBy120Days_covid[a.pmid]) for a in query_covid.articles]
citedBy_count_noncov = [len(citedBy120Days_noncovid[a.pmid]) for a in query_noncovid.articles]
citedBy_count_precov = [len(citedBy120Days_precovid[a.pmid]) for a in query_precovid.articles]
...
print('='*70)
print('CITATIONS PER ARTICLE WITHIN {} DAYS'.format(DAYS_CITEDBY_LIMIT))
print('Citations per article: {:.2f} COVID vs {:.2f} Non-COVID vs {:.2f} Top {} Non-COVID'.format(
                            np.mean(citedBy_count_cov),
                            np.mean(citedBy_count_noncov),
                            np.mean(citedBy_count_noncov_top),
...
# Citations broken down by journal
